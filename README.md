...
- **v0.14.3**：补上收藏闭环。新增“聊天 / 收藏夹”工作区切换，聊天卡片里的“已收藏”支持再次点击取消收藏，收藏后的内容可继续整理为长期记忆、结论或资料库。
- **去重保护**：收藏夹会预先识别当前空间里已存在的 memory / conclusion / library 资产，已整理过的动作会显示完成态并禁用，避免重复沉淀。
- 详细日志见 [`changelogs/0.14.3.md`](changelogs/0.14.3.md)。
