...
- **v0.14.2**：回答卡片正式接入“收藏 / 记住这条 / 沉淀为结论 / 发布到资料库”沉淀闭环，用户可以直接从一条回答出发完成长期资产沉淀。
- **状态回显**：前端会按当前空间回显收藏、记忆、结论、资料库状态，避免用户重复操作，也能看清一条回答是否已经沉淀。
- 详细日志见 [`changelogs/0.14.2.md`](changelogs/0.14.2.md)。
