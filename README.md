...
- **v0.15.0**：升级技术栈，数据库从SQLite升级至postgreSQL
  - 默认主库切换为 PostgreSQL，要求显式配置 `DATABASE_URL`
  - Docker Compose 新增 `postgres` 服务，应用容器默认接入 PostgreSQL
  - SQLite 降级为兼容/备份用途，不再作为默认运行时主库
  - 补齐 PostgreSQL 迁移校验与测试覆盖，确保 Alembic + 应用启动链路可用
