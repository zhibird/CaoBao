...
from app.api.deps import get_conclusion_service
...
def create_conclusion(
    ...
    try:
        item = conclusion_service.create(payload)
...
def update_conclusion(
    ...
    try:
        item = conclusion_service.update(conclusion_id=conclusion_id, payload=payload)
    except EntityNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except DomainValidationError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
...
def confirm_conclusion(
    ...
    try:
        item = conclusion_service.confirm(conclusion_id=conclusion_id, payload=payload)
    except EntityNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except DomainValidationError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
...
def archive_conclusion(
    ...
    try:
        item = conclusion_service.archive(conclusion_id=conclusion_id, payload=payload)
    except EntityNotFoundError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except DomainValidationError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
